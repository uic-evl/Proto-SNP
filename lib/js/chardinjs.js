    ...
    let chardinJs = class chardinJs {
      ...
      start() {
        ...
        for (i = 0, len = ref.length; i < len; i++) {
          ...
          if(this._show_element(el,pos)) svg_elements.push({element: el, position: pos});
        }
        if(this.isSVG){
          this._add_svg_overlay_layer(svg_elements)
        }
        else {
      ...
      toggle() { return (!this._overlay_visible()) ? this.start() : this.stop(); }
      ...
      _add_svg_overlay_layer(elements) {
        ...
        overlay_layer += mask + "</mask></defs><rect x=\"0\" y=\"0\" class=\"svgOverlay\"></rect></svg>";
