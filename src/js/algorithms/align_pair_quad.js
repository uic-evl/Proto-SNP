...
//let alignment = new AlignPairQuad();
//alignment.initializeMatrix(sequenceArrayM, sequenceArrayN, scoreSet);
//alignment.fillMatrix();
//alignment.align();
//let alignedSequenceStringM = alignment.getAlignedM();
//let alignedSequenceStringN = alignment.getAlignedN();
...
function dumpMatrix () {
    ...
    for (let i = 0; i < this.nodes.length; i++) {
        for (let j = 0; j < this.nodes[i].length; j++) {
            let traceI = this.nodes[i][j].tracebackI;
            let traceJ = this.nodes[i][j].tracebackJ;
            ...
            let output = "(" + i + "," + j + ")[" + traceI + "," + traceJ + "]=" + this.nodes[i][j].value;
...
function align() {
    ...
    let currentI = this.nodes.length - 1;
    let currentJ = this.nodes[0].length - 1;

    let currentNode = this.nodes[this.nodes.length - 1][this.nodes[0].length - 1];
